...
mod tests {
    ...
    // };
    // use fuel_tx::{
    //     ConsensusParameters, Input, Output, TransactionBuilder, TxId, TxPointer,
    ...
    //     );
    //     test_builder.builder.add_output(Output::change(secret1_address.clone(), 0,
    // base_asset_id));     test_builder.builder.add_output(Output::coin(secret2_address.
    // clone(), 1, base_asset_id));     let tx1 = test_builder.build().transaction().clone();
    //     let tx1: fuel_tx::Transaction = fuel_tx::Transaction::Script(tx1);
    //     let mut tx_raw_data_vec = vec![];
    //     tx_raw_data_vec.extend_from_slice(tx1.to_bytes().as_slice());
    //     let tx_raw_data_bytes = Bytes::from(tx_raw_data_vec);
