...
.PHONY: fluent_run
fluent_run:
	cargo run -p reth --bin reth $(FLUENT_NODE_BUILD_PROFILE) --features generate-genesis -- --color=never --chain=dev node --datadir=./$(FLUENT_DATADIR) --dev --full --http --port=30305 --engine.persistence-threshold 0 --engine.memory-block-buffer-target 0
